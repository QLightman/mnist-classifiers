...
train_data = train_data.reshape(60000,2025)
train_data=train_data*1.0/255

#train_data=np.load("hog_1818/60000.npy")
#train_data2=np.load("20_20/60000_rotate.npy")
#train_data2=train_data2[:10000]
...
# train_data=np.reshape(train_data,(60000,-1))
# test_data=np.reshape(test_data,(10000,-1))

old_test_label = np.fromfile("mnist_test_label",dtype=np.uint8)
old_train_label = np.fromfile("mnist_train_label",dtype=np.uint8)
#old_train_label=np.concatenate((old_train_label,old_train_label),axis=0)
test_label=np.zeros([10000,10])
for index,value in enumerate(old_test_label):
    test_label[index][value]=1
train_label=np.zeros([60000,10])
for index,value in enumerate(old_train_label):
    train_label[index][value]=1
...
with tf.Session() as sess:
    ...
    for epoch in range(total_epochs):
        ...
        for step in range(total_batch):
            ...
            batch_x=train_data[step*100:step*100+100]
            batch_y=train_label[step*100:step*100+100]
